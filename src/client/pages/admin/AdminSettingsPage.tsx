 ...
 * Three sections wired to the correct config keys and lookup values from
 * shared constants. No data fetching yet — wire up to the config and admin
 * endpoints when those services are implemented.
 */
...
const AdminSettingsPage: React.FC = () => {
  return (
    <div data-testid="admin-settings-page">
      ...
      {/* ── System Config ── */}
      <section aria-labelledby="system-config-heading" data-testid="settings-system-config">
        <h2 id="system-config-heading">System Configuration</h2>
        <p>
          Core parameters used by the quote generation engine. Changes apply to all future
          auto-generated quotes.
        </p>

        <form aria-label="System configuration" data-testid="system-config-form">
          <div>
            <label htmlFor={SMARTQUOTE_CONFIG_KEYS.HOURS_PER_DAY}>Hours Per Day</label>
            <input
          ...
          <div>
            <label htmlFor={SMARTQUOTE_CONFIG_KEYS.VELOCITY_MULTIPLIER}>Velocity Multiplier</label>
            <input
          ...
          <button type="submit" disabled data-testid="system-config-save-btn">
            Save Configuration
          </button>
        </form>
      </section>
      ...
      {/* ── Rate Profiles ── */}
      <section aria-labelledby="rate-profiles-heading" data-testid="settings-rate-profiles">
        <h2 id="rate-profiles-heading">Rate Profiles</h2>
        <p>
          Define hourly rates by ticket type and severity. Business hours and after-hours rates can
          be set independently.
        </p>

        <button type="button" disabled data-testid="add-rate-profile-btn">
          Add Rate Profile
        </button>

        <table aria-label="Rate profiles" data-testid="rate-profiles-table">
          <thead>
            <tr>
          ...
          <tbody>
            {STUB_RATE_PROFILES.map((profile) => (
              <tr key={profile.id} data-testid={`rate-row-${profile.id}`}>
                ...
                <td>
                  <button type="button" disabled data-testid={`rate-edit-${profile.id}`}>
                    Edit
                  </button>
                  <button type="button" disabled data-testid={`rate-delete-${profile.id}`}>
                    Delete
                  </button>
                </td>
              </tr>
      ...
      {/* ── User Management ── */}
      <section aria-labelledby="user-management-heading" data-testid="settings-user-management">
        ...
        <button type="button" disabled data-testid="add-user-btn">
          Add User
        </button>

        <table aria-label="Users" data-testid="users-table">
          <thead>
            <tr>
          ...
          <tbody>
            <tr data-testid="users-table-empty-row">
              <td colSpan={5}>User management coming soon.</td>
            </tr>
          </tbody>
        </table>
      </section>
